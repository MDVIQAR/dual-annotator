...
ONNX inference entrypoint for UNet segmentation.
Launched by InferWorker as a subprocess.

Stdout protocol:
  PROGRESS N
  RESULT {name}|{abs_path_to_combined_image}
  DONE total=N
  [ERROR] message
"""
...
def preprocess(img_bgr, in_channels, image_width, image_height):
    ...
    img_f = (img_f - 0.5) / 0.5          # same normalisation as training
    return img_f.transpose(2, 0, 1)[None] # 1, C, H, W


def colorise_mask(mask_hw, out_classes):
    scale = max(255 // max(out_classes - 1, 1), 1)
    colored = cv2.applyColorMap((mask_hw * scale).astype(np.uint8), cv2.COLORMAP_JET)
    return colored
...
def main():
    try:
        ...
        for i, img_name in enumerate(images):
            ...
            logits = sess.run(None, {input_name: batch})[0]  # 1, C, H, W
            ...
            combined  = np.concatenate([orig_display, colored, overlay], axis=1)
